...
fromInternal ::
     ContentBaseURI -> ContentId -> Internal.DeepZoomImage -> DeepZoomImage
fromInternal baseURI cId dzi =
  DeepZoomImage
  ...
  where
    ...
    path =
      fromMaybe
        (error
           "ZoomHub.API.Types.DeepZoomImage.fromInternal: Failed to parse DZI path.")
...
-- Types
newtype DeepZoomImageURI = DeepZoomImageURI
  { unDeepZoomImageURI :: URI
  } deriving (Eq)
