...
type API
  ...
  -- Meta
   = "health" :> Get '[ HTML] String :<|> "version" :> Get '[ HTML] String
  -- JSONP: ID
      :<|> "v1" :> "content" :> Capture "id" ContentId :> RequiredQueryParam "callback" Callback :> Get '[ JavaScript] (JSONP (NonRESTfulResponse Content))
  -- JSONP: Error: ID
      :<|> "v1" :> "content" :> Capture "id" String :> RequiredQueryParam "callback" Callback :> Get '[ JavaScript] (JSONP (NonRESTfulResponse String))
  -- JSONP: URL
      :<|> "v1" :> "content" :> RequiredQueryParam "url" ContentURI :> RequiredQueryParam "callback" Callback :> Get '[ JavaScript] (JSONP (NonRESTfulResponse Content))
  -- JSONP: Error: URL
      :<|> "v1" :> "content" :> QueryParam "url" String :> RequiredQueryParam "callback" Callback :> Get '[ JavaScript] (JSONP (NonRESTfulResponse String))
  -- API: RESTful: ID
      :<|> "v1" :> "content" :> Capture "id" ContentId :> Get '[ JSON] Content
  -- API: RESTful: Error: ID
      :<|> "v1" :> "content" :> Capture "id" String :> Get '[ JSON] Content
  -- API: RESTful: URL
      :<|> "v1" :> "content" :> RequiredQueryParam "url" ContentURI :> Get '[ JSON] Content
  -- API: RESTful: Error: URL
      :<|> "v1" :> "content" :> QueryParam "url" String :> Get '[ JSON] Content
  -- Embed
      :<|> Capture "embedId" EmbedId :> QueryParam "id" String :> QueryParam "width" EmbedDimension :> QueryParam "height" EmbedDimension :> Get '[ JavaScript] Embed
  -- Web: View
      :<|> Capture "viewId" ContentId :> Get '[ HTML] ViewContent :<|> RequiredQueryParam "url" ContentURI :> Get '[ HTML] ViewContent
...
server config
    -- Meta
 =
  health :<|> version (Config.version config) :<|>
...
jsonpInvalidContentId ::
     String -> Callback -> Handler (JSONP (NonRESTfulResponse String))
...
jsonpInvalidRequest ::
     Maybe String -> Callback -> Handler (JSONP (NonRESTfulResponse String))
jsonpInvalidRequest maybeURL callback =
  case maybeURL of
    Nothing ->
      return $ mkJSONP callback (mkNonRESTful400 apiMissingIdOrURLMessage)
    Just _ -> return $ mkJSONP callback (mkNonRESTful400 invalidURLErrorMessage)

-- API: RESTful
restContentById ::
     BaseURI -> ContentBaseURI -> DatabasePath -> ContentId -> Handler Content
...
webEmbed ::
  ...
  -> Maybe EmbedDimension
  -> Handler Embed
webEmbed baseURI contentBaseURI staticBaseURI dbPath viewerScript embedId maybeId width height = do
  maybeContent <- liftIO $ withConnection dbPath (getById' contentId dbPath)
  case maybeContent of
...
noNewContentError err = throwError . err $ noNewContentErrorMessage
...
redirectURI pathPrefix baseURI contentId =
  (fromJust . parseRelativeReference $ pathPrefix ++ unId contentId) `relativeTo`
  unBaseURI baseURI
