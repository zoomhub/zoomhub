...
spec =
  beforeAll_ setupDatabase $ around withDatabaseConnection $ do
    describe "initialize"
      $ it "should return initialized content"
      $ \conn -> do
    ...
    describe "markAsActive"
      $ it "should mark content as active"
      $ \conn -> do
        (mContent, _) <- runPQ (initialize testURL) conn
        case mContent of
          Just content -> do
    ...
    describe "markAsSuccess"
      $ it "should mark content as successful"
      $ \conn -> do
        (mContent, _) <- runPQ (initialize testURL) conn
        case mContent of
          Just content -> do
    ...
    describe "markAsFailure"
      $ it "should mark content as failure"
      $ \conn -> do
        (mContent, _) <- runPQ (initialize testURL) conn
        case mContent of
          Just content -> do
    ...
    describe "resetAsInitialized"
      $ it "should reset content as initialized"
      $ \conn -> do
        (mContent, _) <- runPQ (initialize testURL) conn
        case mContent of
          Just content -> do
    ...
    describe "dequeueNextUnprocessed" $ do
      context "when two content were initialized at the same time"
        $ it "should return the one that is more popular (more views) and mark active"
        $ \conn -> do
          currentTime <- safeGetCurrentTime
          let minutes n = n * 60
      ...
      context "when there is no content"
        ...
        $ \conn -> do
          ...
          result `shouldBe` Nothing
    describe "getById"
      $ it "should return item by hash ID"
      $ \conn -> do
        (mContent, _) <- runPQ (initialize testURL) conn
        case mContent of
          Just content -> do
          ...
          Nothing ->
            expectationFailure "expected content to be initialized"
    describe "getById'"
      $ it "should increase number of views"
      $ \conn -> do
        (mContent, _) <- runPQ (initialize testURL) conn
        case mContent of
          Just content -> do
    ...
    describe "getNextUnprocessed" $ do
      context "when two content were initialized at the same time"
        $ it "should return the one that is more popular (more views)"
        $ \conn -> do
          currentTime <- safeGetCurrentTime
          let minutes n = n * 60
              -- HACK: Hard-coded content IDs set by database trigger
    ...
    describe "getExpiredActive"
      $ it "should return active content that has expired"
      $ \conn -> do
        currentTime <- safeGetCurrentTime
        let minutes n = n * 60
            -- HACK: Hard-coded content IDs set by database trigger
